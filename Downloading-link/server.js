...
app.get("/d/:id", async (req, res) => {
  ...
  if (USE_BLOB && meta.fileKey) {
    // download=1 makes the CDN answer with Content-Disposition: attachment, so the
    // browser saves the file instead of rendering it inline. It sits outside the
    // signed payload, so appending it does not invalidate the signature.
    return res.redirect(`${await signedUrlFor(meta.fileKey)}&download=1`);
  }
